...
float2 amod(float2 p , float m) { float a = mod(atan2(p.y , p.x) , m) - m * .5; return float2 (cos(a) , sin(a)) * length(p); }
...
float pales(float2 uv , float screen , float number) {
  ...
  return floor(smoothstep(.1 , .2 , cos(atan2(uv.x , uv.y) * number)));
 }
...
fragColor = float4 ((pow(col , float3 (1. / 2.2 , 1. / 2.2 , 1. / 2.2))).x , (pow(col , float3 (1. / 2.2 , 1. / 2.2 , 1. / 2.2))).y , (pow(col , float3 (1. / 2.2 , 1. / 2.2 , 1. / 2.2))).z , 1.0);
return fragColor;
}
