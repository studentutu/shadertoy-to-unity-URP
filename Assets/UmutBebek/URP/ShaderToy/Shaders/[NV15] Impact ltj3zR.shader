...
float3 planet = float3 (0. , 0. , 0.);

const float3 sun = float3 (0. , 0. , 0.);
...
float displacement(float3 p)
 {
    ...
    // float m = .1412 ; 
  float f;
  f = 0.5000 * planetNoise(p); p = mul(m , p) * 2.02;
  f += 0.2500 * planetNoise(p); p = mul(m, p) * 2.03;
  f += 0.1250 * planetNoise(p); p = mul(m, p) * 2.01;
    ...
    // for ( int i = 0 ; i < NUM_PLANETS ; i ++ ) { 
     ...
     float impact(float2 p , float stillness) {
         ...
         for (int i = 0; i < 58; i++) {
             ...
             for (int j = 0; j < 3; j++) {
                 ...
                 f += inBox(p , float2 ((_ScreenParams / 2.).x, (_ScreenParams / 2.).y) + lu , size);
     ...
     float4 overlayFog(float3 ro , float3 rd , float2 screenPos , float hit) {
         ...
         // float nSize = .000002 * hit ; 
             // float n = ( noise ( float3 ( ( 2.0 * screenPos ).x , ( 2.0 * screenPos ).y , abs ( sin ( _Time.y * 10. ) ) * .1 ) ) * nSize ) - .5 * nSize ; 
        for (int i = 0; i < FOG_STEPS; i++) {
            ...
            if (m.x < 0.0) { return float4 ((col).x , (col).y , (col).z , lum) / float(FOG_STEPS); }
          ...
          // if ( fleck > .8 ) { return float4 ( ( float3 ( .2 , 0. , 0. ) * col / float ( i ) ).x , ( float3 ( .2 , 0. , 0. ) * col / float ( i ) ).y , ( float3 ( .2 , 0. , 0. ) * col / float ( i ) ).z , lum ) ; } 
     ...
     return float4 ((col).x , (col).y , (col).z , lum) / float(FOG_STEPS);
...
float3x3 camMat = calcLookAtMatrix(ro , ta , 0.0); // 0.0 is the camera roll 
  ...
  // create view ray 
 float3 rd = normalize(mul(camMat , float3 ((p.xy).x , (p.xy).y , 2.0))); // 2.0 is the lens length 
...
// seeing 
if (timeInLoop < impactTime + whiteTime) { col += float3 (10. * (impactTime + whiteTime - timeInLoop) , 10. * (impactTime + whiteTime - timeInLoop) , 10. * (impactTime + whiteTime - timeInLoop)); }
...
if (timeInLoop < impactTime) {
    ...
    // col = float3 ( fragCoord.x , fragCoord.x , fragCoord.x ) ; 
     // col = SAMPLE_TEXTURE2D ( _Channel0 , sampler_Channel0 , sin ( fragCoord.xy * 10. ) ) .xyz ; // float3 ( 1. , 1. , 1. ) ; 
 }

 fragColor = min(fadeOut , fadeIn) * float4 ((col).x , (col).y , (col).z , 1.0);
