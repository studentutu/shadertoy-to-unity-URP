Shader "UmutBebek/URP/ShaderToy/Elevated MdX3Rr BufferA"
{
    Properties
    {
...
//float4 pointSampleTex2D(Texture2D sam, SamplerState samp, float2 uv, float4 st, float lod)
//{
//    float2 snappedUV = ((float2)((int2)(uv * st.zw + float2(1, 1))) - float2(0.5, 0.5)) * st.xy;
...
//    // texel fetch version 
//   int2 p = int2 (floor(x));
//   float2 snappedUV = ((float2)((int2)(((p + int2 (0, 0)) & 255) * _ScreenParams.zw + float2(1, 1))) - float2(0.5, 0.5)) * _ScreenParams.xy;
//   float a = SAMPLE_TEXTURE2D(_Channel0, sampler_Channel0, float4(snappedUV.x, snappedUV.y, 0, 0)).x;
//   //float a = pointSampleTex2D(_Channel0 , sampler_Channel0 , (p + int2 (0 , 0)) & 255 ).x;
//   snappedUV = ((float2)((int2)(((p + int2 (1, 0)) & 255) * _ScreenParams.zw + float2(1, 1))) - float2(0.5, 0.5)) * _ScreenParams.xy;
//   float b = SAMPLE_TEXTURE2D(_Channel0, sampler_Channel0, float4(snappedUV.x, snappedUV.y, 0, 0)).x;
//    //float b = pointSampleTex2D(_Channel0 , sampler_Channel0 , (p + int2 (1 , 0)) & 255 ).x;
//   snappedUV = ((float2)((int2)(((p + int2 (0, 1)) & 255) * _ScreenParams.zw + float2(1, 1))) - float2(0.5, 0.5)) * _ScreenParams.xy;
//   float c = SAMPLE_TEXTURE2D(_Channel0, sampler_Channel0, float4(snappedUV.x, snappedUV.y, 0, 0)).x;
//    //float c = pointSampleTex2D(_Channel0 , sampler_Channel0 , (p + int2 (0 , 1)) & 255 ).x;
//   snappedUV = ((float2)((int2)(((p + int2 (1, 1)) & 255) * _ScreenParams.zw + float2(1, 1))) - float2(0.5, 0.5)) * _ScreenParams.xy;
//   float d = SAMPLE_TEXTURE2D(_Channel0, sampler_Channel0, float4(snappedUV.x, snappedUV.y, 0, 0)).x;
//    //float d = pointSampleTex2D(_Channel0 , sampler_Channel0 , (p + int2 (1 , 1)) & 255 ).x;
////#else 
...
float terrainH(in float2 x)
 {
    ...
    float b = 1.0;
     ...
     for (int i = 0; i < 16; i++)
     {
        ...
        a += b * n.x / (1.0 + dot(d , d));
          b *= 0.5;
        p = mul(float2x2(0.8, -0.6, 0.6, 0.8), p) * 2.0;
     }
...
float fbm(float2 p)
 {
    float f = 0.0;
    f += 0.5000 * SAMPLE_TEXTURE2D(_Channel0 , sampler_Channel0 , p / 256.0).x; 
    p = mul(float2x2(0.8, -0.6, 0.6, 0.8) , p) * 2.02;
    f += 0.2500 * SAMPLE_TEXTURE2D(_Channel0 , sampler_Channel0 , p / 256.0).x; 
    p = mul(float2x2(0.8, -0.6, 0.6, 0.8) , p) * 2.03;
    f += 0.1250 * SAMPLE_TEXTURE2D(_Channel0 , sampler_Channel0 , p / 256.0).x; 
    p = mul(float2x2(0.8, -0.6, 0.6, 0.8) , p) * 2.01;
 ...
 for (int n = 0; n < AA; n++)
  {
    ...
    // camera ray 
   float3 rd = mul(cam , normalize(float3 (s , fl)));
