Shader "UmutBebek/URP/ShaderToy/Simplex Trabeculae 3tXcRs"
{
    Properties
    {
        ...
        [HideInInspector]iMouse("Mouse", Vector) = (0,0,0,0)
            ...
            _Lod("Lod",float) = 0
            _AR("AR Mode",float) = 0*/
            STEPS("STEPS", float) = 256
TMAX("TMAX", float) = 100.
PRECIS("PRECIS", float) = .0001
shaded("shaded", float) = 0
...
float2 fragCoord = ((input.screenPos.xy) / (input.screenPos.w + FLT_MIN)) * _ScreenParams.xy;
  ...
  col += .004 * (SAMPLE_TEXTURE2D(_Channel0 , sampler_Channel0 , mul(fragCoord * .001 , r(.2)) + .1).xxx - .5)
         + .008 * (SAMPLE_TEXTURE2D(_Channel0 , sampler_Channel0 , mul(fragCoord * .002, r(.3)) + .1).xxx - .5)
         + .015 * (SAMPLE_TEXTURE2D(_Channel0 , sampler_Channel0 , mul(fragCoord * .004 , r(.5)) + .1).xxx - .5)
         + .03 * (SAMPLE_TEXTURE2D(_Channel0 , sampler_Channel0 , mul(fragCoord * .008 , r(.7)) + .1).xxx - .5);
