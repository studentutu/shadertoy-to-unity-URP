Shader "UmutBebek/URP/ShaderToy/Mario World 1-1 XtlSD7"
{
    Properties
    {
        ...
        MARIO_SPEED("MARIO_SPEED", float) = 89.0
GOOMBA_SPEED("GOOMBA_SPEED", float) = 32.0
INTRO_LENGTH("INTRO_LENGTH", float) = 2.0
...
void SpriteBlock(inout float3 color , float x , float y)
 {
    // black 
   float idx = 1.0;
...
void SpriteHill(inout float3 color , float x , float y)
 {
    float idx = 0.0;
...
void SpritePipe(inout float3 color , float x , float y , float h)
 {
    float offset = h * 16.0;
   ...
   idx = (x < 5.0 || x > 11.0) && y == 29.0 + offset ? 2.0 : idx;
    idx = frac(x * 0.5 + y * 0.5) == 0.5 && x > 22.0 && ((x < 26.0 && y < 17.0 + offset) || (x < 28.0 && y > 17.0 + offset && y < 30.0 + offset)) ? 2.0 : idx;
...
void SpriteCloud(inout float3 color , float x , float y , float isBush)
 {
     float idx = 0.0;
...
void SpriteFlag(inout float3 color , float x , float y)
 {
     float idx = 0.0;
...
void SpriteCastleFlag(inout float3 color , float x , float y)
 {
     float idx = 0.0;
...
void SpriteGoomba(inout float3 color , float x , float y , float frame)
 {
     float idx = 0.0;
...
void SpriteKoopa(inout float3 color , float x , float y , float frame)
 {
     float idx = 0.0;
...
void SpriteQuestion(inout float3 color , float x , float y , float t)
 {
     float idx = 0.0;
     ...
     color = idx == 1.0 ? RGB(0 , 0 , 0) : color;
     color = idx == 2.0 ? RGB(231 , 90 , 16) : color;
     color = idx == 3.0 ? lerp(RGB(255 , 165 , 66) , RGB(231 , 90 , 16) , t) : color;
 }

void SpriteMushroom(inout float3 color , float x , float y)
 {
     float idx = 0.0;
     idx = y == 15.0 ? (x <= 7.0 ? 40960.0 : 10.0) : idx;
...
void SpriteGround(inout float3 color , float x , float y)
 {
     float idx = 0.0;
...
void SpriteFlagpoleEnd(inout float3 color , float x , float y)
 {
     float idx = 0.0;
...
void SpriteMario(inout float3 color , float x , float y , float frame)
 {
    float idx = 0.0;
...
void SpriteCoin(inout float3 color , float x , float y , float frame)
 {
    float idx = 0.0;
     if (frame == 0.0)
     {
...
void SpriteBrick(inout float3 color , float x , float y)
 {
...
void DrawCastle(inout float3 color , float x , float y)
 {
...
void DrawKoopa(inout float3 color , float x , float y , float frame)
 {
    if (x >= 0.0 && x <= 15.0)
     {
...
void KoopaWalk(inout float3 color , float worldX , float worldY , float time , float frame , float startX)
 {
    float x = worldX - startX + floor(time * GOOMBA_SPEED);
    DrawKoopa(color , x , worldY - 16.0 , frame);
 }

void DrawHitQuestion(inout float3 color , float questionX , float questionY , float time , float questionT , float questionHitTime)
 {
...
void DrawW(inout float3 color , float x , float y)
 {
...
void DrawO(inout float3 color , float x , float y)
 {
    if (x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0)
     {
...
void DrawR(inout float3 color , float x , float y)
 {
    if (x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0)
     {
        if ((x <= 3.0)
...
void DrawL(inout float3 color , float x , float y)
 {
    if (x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0)
     {
        if (x <= 3.0 || y <= 1.0)
...
void DrawD(inout float3 color , float x , float y)
 {
    if (x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0)
     {
         color = RGB(255 , 255 , 255);
...
void Draw1(inout float3 color , float x , float y)
 {
    if (x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0)
     {
        if ((y <= 1.0)
...
void DrawM(inout float3 color , float x , float y)
 {
    if (x >= 0.0 && x < 14.0 && y >= 0.0 && y < 14.0)
     {
        if (y >= 4.0 && y <= 7.0)
...
void DrawIntro(inout float3 color , float x , float y , float screenWidth , float screenHeight)
 {
    color = RGB(0 , 0 , 0);
...
void DrawGame(inout float3 color , float time , float pixelX , float pixelY , float screenWidth , float screenHeight)
 {
...
if (time > mushroomPauseStart + 0.7)
 {
    float t = time - mushroomPauseStart - 0.7;
     if (mod(t , 0.2) <= lerp(0.0 , 0.2 , clamp(t / 1.3 , 0.0 , 1.0)))
     {
         // super mario offset 
        marioFrame += 4.0;
     }
...
float resMultX = floor(_ScreenParams.x / 224.0);
float resMultY = floor(_ScreenParams.y / 192.0);
float resRcp = 1.0 / max(min(resMultX , resMultY) , 1.0);

float time = _Time.y;
float screenWidth = floor(_ScreenParams.x * resRcp);
float screenHeight = floor(_ScreenParams.y * resRcp);
