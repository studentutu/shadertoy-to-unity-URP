...
float voroni(in float2 _st , in float _offset)
 {
    ...
    float2 m_point; // minimum pointExtended 

    for (int j = -1; j <= 1; j++) {
        for (int i = -1; i <= 1; i++) {
            float2 neighbor = float2 (float(i) , float(j));
            float2 pointExtended = float2 (random(i_st + neighbor), random(i_st + neighbor));
            pointExtended = 0.5 + 0.5 * sin(_offset + 6.2831 * pointExtended);
            float2 diff = neighbor + pointExtended - f_st;
            float dist = length(diff);

            if (dist < m_dist) {
                m_dist = dist;
                m_point = pointExtended;
             }
         }
     }
