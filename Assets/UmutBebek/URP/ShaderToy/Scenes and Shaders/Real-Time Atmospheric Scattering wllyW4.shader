...
PLANET_RADIUS("PLANET_RADIUS", float) = 6.371
ATMOSPHERE_HEIGHT("ATMOSPHERE_HEIGHT", float) = 0.100
RAYLEIGH_HEIGHT("RAYLEIGH_HEIGHT", float) = 0.08
MIE_HEIGHT("MIE_HEIGHT", float) = 0.012
OZONE_PEAK_LEVEL("OZONE_PEAK_LEVEL", float) = 0.30
OZONE_FALLOFF("OZONE_FALLOFF", float) = 0.03
G("G", float) = 0.75
SAMPLES("SAMPLES", float) = 4
SUN_ILLUMINANCE("SUN_ILLUMINANCE", float) = 128000.0
MOON_ILLUMINANCE("MOON_ILLUMINANCE", float) = 0.32
SPACE_ILLUMINANCE("SPACE_ILLUMINANCE", float) = 0.01
ATMOSPHERE_RADIUS("ATMOSPHERE_RADIUS", float) = 1
...
float MIE_HEIGHT;
float OZONE_PEAK_LEVEL;
float OZONE_FALLOFF;
float G;
float SAMPLES;
float SUN_ILLUMINANCE;
float MOON_ILLUMINANCE;
float SPACE_ILLUMINANCE;
...
for (int i = 0; i < SAMPLES; i++) {
  ...
  float aaa = exp(-(BETA_RAY *
      (opticalDepth.x + lightOpticalDepth.x) + BETA_MIE *
      (opticalDepth.y + lightOpticalDepth.y) + BETA_OZONE * (opticalDepth.z + lightOpticalDepth.z)));
float3 scattered = float3(aaa, aaa, aaa);
...
float2 fragCoord = ((input.screenPos.xy) / (input.screenPos.w + FLT_MIN)) * _ScreenParams.xy;
    ...
    float2 mouse = iMouse.xy / _ScreenParams.xy;
     ...
     float3 sunDir = 
         all(iMouse.xy == float2 (0.0 , 0.0)) ? float3 (0.0 , -0.03 , -1.0) : viewDir(mouse , ratio);
    dir = normalize(dir);
