Shader "UmutBebek/URP/ShaderToy/Warping - procedural 2 lsl3RH"
{
    Properties
    {
...
float fbm4(float2 p)
 {
    float f = 0.0;
    f += 0.5000 * noise(p); 
    p = mul(float2x2(0.80, 0.60, -0.60, 0.80) , p) * 2.02;
    f += 0.2500 * noise(p); 
    p = mul(float2x2(0.80, 0.60, -0.60, 0.80), p) * 2.03;
    f += 0.1250 * noise(p); 
    p = mul(float2x2(0.80, 0.60, -0.60, 0.80) , p) * 2.01;
    f += 0.0625 * noise(p);
    return f / 0.9375;
 }
...
float fbm6(float2 p)
 {
    ...
    p = mul(float2x2(0.80, 0.60, -0.60, 0.80), p) * 2.04;
...
float func(float2 q , out float4 ron)
 {
    q += 0.03 * sin(float2 (0.27 , 0.23) * _Time.y*5 + length(q) * float2 (4.1 , 4.3));

     float2 o = fbm4_2(0.9 * q);

    o += 0.04 * sin(float2 (0.12 , 0.14) * _Time.y*5 + length(o));
