Shader "UmutBebek/URP/ShaderToy/Himalayas MdGfzh BufferA"
{
    Properties
    {
...
UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
 ...
 float2 fragCoord = ((input.screenPos.xy) / (input.screenPos.w + FLT_MIN)) * _ScreenParams.xy;
     if (resolutionChanged()) {
         float2 vUV = fragCoord / _ScreenParams.xy;
         float3 coord = frac(float3 (vUV + float2 (.2 , 0.62) , .5));
         ...
         col.r = lerp(1. , tilableFbm(coord , 7 , 4.) , mfbm) *
