Shader "UmutBebek/URP/ShaderToy/Interstellar Xdl3D2"
{
    Properties
    {
       ...
       float4 Noise(in int2 x)
        {
            return SAMPLE_TEXTURE2D_LOD(_Channel0 , sampler_Channel0 , (float2 (x)+0.5) / 256.0 , 0);
        }
       ...
       float4 Rand(in int x)
        {
            ...
            return SAMPLE_TEXTURE2D_LOD(_Channel0 , sampler_Channel0 , uv , 0);
        }
      ...
      UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
       ...
       float2 fragCoord = ((input.screenPos.xy) / (input.screenPos.w + FLT_MIN)) * _ScreenParams.xy;
            float3 ray;
            ray.xy = 2.0 * (fragCoord.xy - _ScreenParams.xy * .5) / _ScreenParams.x;
            ray.z = 1.0;
            ...
            fragColor = float4 (ToGamma(col) , 1.0);
        return fragColor;
       }
