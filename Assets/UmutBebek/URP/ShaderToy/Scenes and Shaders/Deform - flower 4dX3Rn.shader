Shader "UmutBebek/URP/ShaderToy/Deform - flower 4dX3Rn"
{
    Properties
    {
...
float2 fragCoord = ((input.screenPos.xy) / (input.screenPos.w + FLT_MIN)) * _ScreenParams.xy;
    float2 p = (2.0 * fragCoord - _ScreenParams.xy) / min(_ScreenParams.y , _ScreenParams.x);

    float a = atan2(p.x , p.y);
    float r = length(p) * (0.8 + 0.2 * sin(0.3 * _Time.y));
