Shader "UmutBebek/URP/ShaderToy/Clover XsXGzn"
{
    Properties
    {
...
float2 fragCoord = ((input.screenPos.xy) / (input.screenPos.w + FLT_MIN)) * _ScreenParams.xy;
     float2 q = 0.6 * (2.0 * fragCoord - _ScreenParams.xy) / min(_ScreenParams.y , _ScreenParams.x);

    float a = atan2(q.x , q.y);
    float r = length(q);
    float s = 0.50001 + 0.5 * sin(3.0 * a + _Time.y);
    float g = sin(1.57 + 3.0 * a + _Time.y);
