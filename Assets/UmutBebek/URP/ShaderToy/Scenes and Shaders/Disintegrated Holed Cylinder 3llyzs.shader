Shader "UmutBebek/URP/ShaderToy/Disintegrated Holed Cylinder 3llyzs"
{
    Properties
    {
        ...
        [HideInInspector]iMouse("Mouse", Vector) = (0,0,0,0)
            ...
            _Lod("Lod",float) = 0
            _AR("AR Mode",float) = 0*/
            sphere1Pos("sphere1Pos", vector) = (0. , 0., 0.)
sphere1Rad("sphere1Rad", float) = .7 // sphere radius 
planePos("planePos", float) = .1
sphere2Pos("sphere2Pos", vector) = (1. , 0., 0.)
sphere2Rad("sphere2Rad", float) = .2
H("H", float) = .2 // skin layer thickness ( % of normalized sphere ) 
sharp("sharp", float) = 0.9 // cloud sharpness ( 0 = ultra sharp ) . 


cam("cam", vector) = (0. , 0. , 10.)
uniform_step("uniform_step", float) = .5
...
float perturb(float2 p , float d , float H) {
  ...
  float n = 2. * fbm(float3 (p / H , t)) - 1.; // perturbation in [ - 1 , 1] 
  return 2. * (d + GAIN * n); // still in [ - 1 , 1] : - ) 
}

// convert [ - 1 , 1] distances into densities 

float dist2dens(float d) { // transition around zeroExtended. Tunable sharpness 
...
void draw_disk(float3 dir , float3 center , float3 normal , float radius , inout float3 c)
 {
    ...
    antialiasing = frac(1e4 * sin(1e4 * dot(dir , float3 (1. , 7.1 , 13.3)))); // Comment to see it without antialiasing 
...
float2 fragCoord = ((input.screenPos.xy) / (input.screenPos.w + FLT_MIN)) * _ScreenParams.xy;
    ...
    draw_disk(normalize(float3 (uv , -1.)) , float3 (0. , 0. , 0.) , float3 (0. , 1. , 0.) , 30. , color);

    fragColor = float4 (min(color , max(float3 (0. , 0. , 0.) , col)) , 1.);
