Shader "UmutBebek/URP/ShaderToy/Skyline XtsSWs"
{
    Properties
    {
        ...
        frameToRenderHQ("frameToRenderHQ", float) = 50.0 // Time in seconds of frame to render 
antialiasingSamples("antialiasingSamples", float) = 16.0 // 16x antialiasing - too much might make the shader compiler angry. 
localTime("localTime", float) = 0.0
seed("seed", float) = 1.0
fade("fade", float) = 1.0
exposure("exposure", float) = 1.0
marchCount("marchCount", float) = 0.0
zeroOne("zeroOne", vector) = (0.0 , 1.0, 0,0)
voxelPad("voxelPad", float) = 0.2
blockRate("blockRate", float) = 20.0
        ...
        SubShader
{
    ...
    float4 iMouse;
    float frameToRenderHQ;
float antialiasingSamples;
float localTime;
float seed;
float fade;
float exposure;
...
float blockRate;
                   ...
                   /* -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
License CC0 - http: // creativecommons.org / publicdomain / zeroExtended / 1.0 /
...
float3 GetEnvMap(float3 rayDir , float3 sunDir)
 {
  ...
  finalColor = lerp(finalColor , (float3 (1.0 , 1.0 , 1.0) + sunCol * 10.0) * 0.75 * saturate((rayDir.y + 0.2) * 5.0) , saturate(n * 0.125));
...
float3 GetEnvMapSkyline(float3 rayDir , float3 sunDir , float height)
 {
    float3 finalColor = GetEnvMap(rayDir , sunDir);

    // Make a skyscraper skyline reflection. 
   float radial = atan2(rayDir.z , rayDir.x) * 4.0;
   ...
   finalColor = lerp(finalColor * float3 (0.1 , 0.07 , 0.05) , finalColor , mask);
...
float cylCap(float3 p , float r , float lenRad)
 {
...
float2 Car(float3 baseCenter , float unique)
 {
    // bottom box 
   float car = sdBox(baseCenter + float3 (0.0 , -0.008 , 0.001) , float3 (0.01 , 0.00225 , 0.0275));
   // top box smooth blended 
  car = smin(car , sdBox(baseCenter + float3 (0.0 , -0.016 , 0.008) , float3 (0.005 , 0.0005 , 0.01)) , -160.0);
  // mirror the z axis to duplicate the cylinders for wheels 
 float3 wMirror = baseCenter + float3 (0.0 , -0.005 , 0.0);
 wMirror.z = abs(wMirror.z) - 0.02;
 float wheels = cylCap((wMirror).zyx , 0.004 , 0.0135);
 // Set materials 
float2 distAndMat = float2 (wheels , 3.0); // car wheels 
 // Car material is some big number that's unique to each car 
 // so I can have each car be a different color 
distAndMat = matmin(distAndMat , float2 (car , 100000.0 + unique)); // car 
return distAndMat;
}
...
float2 CityBlock(float3 p , float2 pint)
 {
  ...
  height += 0.1; // minimum building height 
   // height += sin ( _Time.y + pint.x ) ; // animate the building heights if you're feeling silly 
  height = floor(height * 20.0) * 0.05; // height is in floor units - each floor is 0.05 high. 
   float d = sdBox(baseCenter , float3 (baseRad , height , baseRad)); // large building piece 

   // road 
  d = min(d , p.y);

  // if ( length ( pint.xy ) > 8.0 ) return float2 ( d , mat ) ; // Hack to LOD in the distance 
 ...
 rand2 = floor(rand2 * 20.0) * 0.05; // floor units 
  ...
  d = min(d , sdBox(baseCenter - float3 (0.0 , height , 0.0) , float3 (baseRad , height2 - rand2.y , baseRad * 0.4)));
  d = min(d , sdBox(baseCenter - float3 (0.0 , height , 0.0) , float3 (baseRad * 0.4 , height2 - rand2.x , baseRad)));
  // second building section 
 if (rand2.y > 0.25)
  {
       d = min(d , sdBox(baseCenter - float3 (0.0 , height , 0.0) , float3 (baseRad * 0.8 , height2 , baseRad * 0.8)));
      ...
      if (height2 > 0.0) topWidth = baseRad * 0.8;
        d = max(d , -sdBox(baseCenter - float3 (0.0 , height + height2 , 0.0) , float3 (topWidth - 0.0125 , 0.015 , topWidth - 0.0125)));
   }
  else
   {
     // Cylinder top section of building 
      if (height2 > 0.0) d = min(d , cylCap((baseCenter - float3 (0.0 , height , 0.0)).xzy , baseRad * 0.8 , height2));
 }
 // mini elevator shaft boxes on top of building 
 d = min(d , sdBox(baseCenter - float3 ((rand.x - 0.5) * baseRad , height + height2 , (rand.y - 0.5) * baseRad) ,
                 float3 (baseRad * 0.3 * rand.z , 0.1 * rand2.y , baseRad * 0.3 * rand2.x + 0.025)));
 // mirror another box ( and scale it ) so we get 2 boxes for the price of 1. 
float3 boxPos = baseCenter - float3 ((rand2.x - 0.5) * baseRad , height + height2 , (rand2.y - 0.5) * baseRad);
float big = sign(boxPos.x);
boxPos.x = abs(boxPos.x) - 0.02 - baseRad * 0.3 * rand.w;
 d = min(d , sdBox(boxPos ,
float3 (baseRad * 0.3 * rand.w , 0.07 * rand.y , baseRad * 0.2 * rand.x + big * 0.025)));

 // Put domes on some building tops for variety 
if (rand.y < 0.04)
 {
    d = min(d , length(baseCenter - float3 (0.0 , height , 0.0)) - baseRad * 0.8);
 }

// d = max ( d , p.y ) ; // flatten the city for debugging cars 

// Need to make a material variable. 
float2 distAndMat = float2 (d , 0.0);
// sidewalk box with material 
distAndMat = matmin(distAndMat , float2 (sdBox(baseCenter , float3 (0.35 , 0.005 , 0.35)) , 1.0));
...
float2 DistanceToObject(float3 p)
 {
   ...
   float3 rep = p;
   rep.xz = frac(p.xz); // [0..1] for representing the position in the city block 
   float2 distAndMat = CityBlock(rep , floor(p.xz));
  ...
  if (abs(frac(rep.x) - 0.5) < 0.35)
   {
      p2.x += 0.05;
      p2.xz = p2.zx * float2 (-1.0 , 1.0); // Rotate 90 degrees 
      rep.xz = p2.xz;
      crossStreet = 0.0;
  ...
  float2 carDist = Car(rep , uniqueID); // car distance function 
...
// This basically makes a procedural SAMPLE_TEXTURE2D map for the sides of the buildings. 
// It makes a SAMPLE_TEXTURE2D , a normal for normal mapping , and a mask for window reflection. 
void CalcWindows(float2 block , float3 pos , inout float3 texColor , inout float windowRef , inout float3 normal)
 {
    float3 hue = float3 (Hash21(block) * 0.8 , Hash21(block * 7.89) * 0.4 , Hash21(block * 37.89) * 0.5);
    texColor += hue * 0.4;
    texColor *= 0.75;
    float window = 0.0;
    window = max(window , lerp(0.2 , 1.0 , floor(frac(pos.y * 20.0 - 0.35) * 2.0 + 0.1)));
    if (pos.y < 0.05) window = 1.0;
    float winWidth = Hash21(block * 4.321) * 2.0;
    if ((winWidth < 1.3) && (winWidth >= 1.0)) winWidth = 1.3;
    window = max(window , lerp(0.2 , 1.0 , floor(frac(pos.x * 40.0 + 0.05) * winWidth)));
    window = max(window , lerp(0.2 , 1.0 , floor(frac(pos.z * 40.0 + 0.05) * winWidth)));
    if (window < 0.5)
     {
        windowRef += 1.0;
     }
...
float3 RayTrace(in float2 fragCoord)
 {
    marchCount = 0.0;
    // -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- animate -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- - 
  sunCol = float3 (258.0 , 248.0 , 200.0) / 3555.0;
   sunDir = normalize(float3 (0.93 , 1.0 , 1.0));
  horizonCol = float3 (1.0 , 0.95 , 0.85) * 0.9;
  skyCol = float3 (0.3 , 0.5 , 0.95);
  exposure = 1.0;
  fade = 1.0;
  ...
  // Map uv to [ - 1.0..1.0] 
  float2 uv = fragCoord.xy / _ScreenParams.xy * 2.0 - 1.0;
 uv /= 2.0; // zoom in 
    ...
    // debugging camera 
   float mx = -iMouse.x / _ScreenParams.x * PI * 2.0; // + localTime * 0.05 ; 
    float my = iMouse.y / _ScreenParams.y * 3.14 * 0.5 + PI / 2.0; // + sin ( localTime * 0.3 ) * 0.8 + 0.1 ; // * PI / 2.01 ; 
    camPos = float3 (cos(my) * cos(mx) , sin(my) , cos(my) * sin(mx)) * 7.35; // 7.35 
#else 
  ...
  if (localTime < t1)
   {
      ...
      camPos = float3 (13.0 , 3.3 , -3.5);
      camPos.x -= smoothstep(0.0 , 1.0 , alpha) * 4.8;
      camUp = float3 (0 , 1 , 0);
      camLookat = float3 (0 , 1.5 , 1.5);
   }
else if (localTime < t2)
{
   ...
   camPos = float3 (26.0 , 0.05 + smoothstep(0.0 , 1.0 , alpha) * 0.4 , 2.0);
   camPos.z -= alpha * 2.8;
   camUp = float3 (0 , 1 , 0);
   camLookat = float3 (camPos.x - 0.3 , -8.15 , -40.0);

   sunDir = normalize(float3 (0.95 , 0.6 , 1.0));
   sunCol = float3 (258.0 , 248.0 , 160.0) / 3555.0;
   exposure *= 0.7;
   skyCol *= 1.5;
}
else if (localTime < t3)
{
   ...
   camUp = normalize(float3 (0 , 1 , -0.5 + alpha * 0.5));
   camLookat = float3 (0 , 1.0 , -0.5);
}
else if (localTime < t4)
{
   ...
   camPos = float3 (2.15 - alpha * 0.5 , 0.02 , -1.0 - alpha * 0.2);
   camPos.y += smoothstep(0.0 , 1.0 , alpha * alpha) * 3.4;
   camUp = normalize(float3 (0 , 1 , 0.0));
   camLookat = float3 (0 , 0.5 + alpha , alpha * 5.0);
}
else if (localTime < t5)
{
   ...
   camPos = float3 (-2.0 , 1.3 - alpha * 1.2 , -10.5 - alpha * 0.5);
   camUp = normalize(float3 (0 , 1 , 0.0));
   camLookat = float3 (-2.0 , 0.3 + alpha , -0.0);
   sunDir = normalize(float3 (0.5 - alpha * 0.6 , 0.3 - alpha * 0.3 , 1.0));
   sunCol = float3 (258.0 , 148.0 , 60.0) / 3555.0;
   localTime *= 16.0;
   exposure *= 0.4;
   horizonCol = float3 (1.0 , 0.5 , 0.35) * 2.0;
   skyCol = float3 (0.75 , 0.5 , 0.95);

}
else if (localTime < t6)
{
   fade = 0.0;
   camPos = float3 (26.0 , 100.0 , 2.0);
   camUp = float3 (0 , 1 , 0);
   camLookat = float3 (0.3 , 0.15 , 0.0);
}
#endif 
 ...
 // -- -- -- -- -- -- -- -- -- -- -- -- -- -- - Ray march the scene -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- 
float2 distAndMat; // Distance and material 
...
for (int i = 0; i < 250; i++) // This is the count of the max times the ray actually marches. 
 ...
 float dx = -frac(pos.x);
 if (rayVec.x > 0.0) dx = frac(-pos.x);
 float dz = -frac(pos.z);
 if (rayVec.z > 0.0) dz = frac(-pos.z);
 float nearestVoxel = min(frac(dx / rayVec.x) , frac(dz / rayVec.z)) + voxelPad;
...
if ((t <= maxDepth) || (t == alpha))
  {
    ...
    // sample the current pointExtended and neighboring points , you can use the difference to get 
    // the normal. 
   float3 smallVec = float3 (smallVal , 0 , 0);
   float3 normalU = float3 (dist - DistanceToObject(pos - smallVec.xyy).x ,
...
float3 nudgePos = pos + normal * 0.002; // don't start tracing too close or inside the object 
  for (int i = 0; i < 40; i++)
 {
    ...
    float walk = tempDist;
    float dx = -frac(shadowPos.x);
    if (sunDir.x > 0.0) dx = frac(-shadowPos.x);
    float dz = -frac(shadowPos.z);
    if (sunDir.z > 0.0) dz = frac(-shadowPos.z);
    float nearestVoxel = min(frac(dx / sunDir.x) , frac(dz / sunDir.z)) + smallVal;
...
// make a few frequencies of noise to give it some SAMPLE_TEXTURE2D 
float n = 0.0;
n += noise(pos * 32.0);
n += noise(pos * 64.0);
n += noise(pos * 128.0);
...
// -- -- -- Calculate SAMPLE_TEXTURE2D color -- -- -- 
float2 block = floor(pos.xz);
float3 texColor = float3 (0.95 , 1.0 , 1.0);
texColor *= 0.8;
float windowRef = 0.0;
// SAMPLE_TEXTURE2D map the sides of buildings 
if ((normal.y < 0.1) && (distAndMat.y == 0.0))
 {
    float3 posdx = ddx(pos);
    float3 posdy = ddy(pos);
    float3 posGrad = posdx * Hash21(uv) + posdy * Hash21(uv * 7.6543);

    // Quincunx antialias the building SAMPLE_TEXTURE2D and normal map. 
    // I guess procedural textures are hard to mipmap. 
   float3 colTotal = float3 (0.0 , 0.0 , 0.0);
   float3 colTemp = texColor;
   float3 nTemp = float3 (0.0 , 0.0 , 0.0);
...
else
 {
  ...
  texColor = lerp(texColor , float3 (1.0 , 0.8 , 0.3) , yellowLine);
 ...
 whiteLine *= saturate(1.0 - (frac(zroad * 8.0) - 0.5) * 280.0); // dotted line 
 whiteLine *= saturate(1.0 - (frac(xroad * 8.0) - 0.5) * 280.0);
 texColor = lerp(texColor , float3 (0.5 , 0.5 , 0.5) , whiteLine);
 ...
 // crosswalk 
float crossWalk = saturate(1.0 - (frac(xroad * 40.0) - 0.5) * 280.0);
crossWalk *= saturate((zroad - 0.15) * 880.0);
crossWalk *= saturate((-zroad + 0.21) * 880.0) * (1.0 - road);
crossWalk *= n * n;
texColor = lerp(texColor , float3 (0.25 , 0.25 , 0.25) , crossWalk);
crossWalk = saturate(1.0 - (frac(zroad * 40.0) - 0.5) * 280.0);
...
texColor = lerp(texColor , float3 (0.25 , 0.25 , 0.25) , crossWalk);

 {
    // sidewalk cracks 
   float sidewalk = 1.0;
   float2 blockSize = float2 (100.0, 100.0);
   if (pos.y > 0.1) blockSize = float2 (10.0 , 50);
  ...
  sidewalk = saturate(lerp(0.7 , 1.0 , sidewalk));
...
if (distAndMat.y == 3.0)
 {
    texColor = float3 (0.05 , 0.05 , 0.05);
 }

// apply noise 
texColor *= float3 (1.0 , 1.0 , 1.0) * n * 0.05;
texColor *= 0.7;
...
if (distAndMat.y >= 100.0)
 {
    // car SAMPLE_TEXTURE2D and windows 
   texColor = float3 (Hash11(distAndMat.y) * 1.0 , Hash11(distAndMat.y * 8.765) , Hash11(distAndMat.y * 17.731)) * 0.1;
   texColor = pow(abs(texColor) , float3 (0.2 , 0.2 , 0.2)); // bias toward white 
   texColor = max(float3 (0.25 , 0.25 , 0.25) , texColor); // not too saturated color. 
   ...
   float2 dirNorm = abs(normalize(normal.xz));
...
float3 lightColor = float3 (100.0, 100.0, 100.0) * sunCol * saturate(dot(sunDir , normal)) * sunShadow;
...
if (distAndMat.y >= 100.0)
 {
   ...
   yfade *= (saturate(1.0 - abs(ddx(windowMask) * ddy(windowMask)) * 250.995));
   finalColor += GetEnvMapSkyline(ref , sunDir , pos.y - 1.5) * 0.3 * yfade * max(0.4 , sunShadow);
   finalColor += saturate(SAMPLE_TEXTURECUBE_LOD(_Channel0 , sampler_Channel0 , ref,0).xyz - 0.35) * 0.15 * max(0.2 , sunShadow);
}
// reflections for building windows 
if (windowRef != 0.0)
 {
    finalColor *= lerp(1.0 , 0.6 , windowRef);
    float yfade = max(0.01 , min(1.0 , ref.y * 100.0));
    finalColor += GetEnvMapSkyline(ref , sunDir , pos.y - 0.5) * 0.6 * yfade * max(0.6 , sunShadow) * windowRef; // * ( windowMask * 0.5 + 0.5 ) ; 
    finalColor += saturate(SAMPLE_TEXTURECUBE_LOD(_Channel0 , sampler_Channel0 , ref,0).xyz - 0.35) * 0.15 * max(0.25 , sunShadow) * windowRef;
 }
finalColor *= 0.9;
// fog that fades to reddish plus the sun color so that fog is brightest towards sun 
float3 rv2 = rayVec;
rv2.y *= saturate(sign(rv2.y));
float3 fogColor = GetEnvMap(rv2 , sunDir);
fogColor = min(float3 (9.0, 9.0, 9.0) , fogColor);
finalColor = lerp(fogColor , finalColor , exp(-t * 0.02));

// visualize length of gradient of distance field to check distance field correctness 
// finalColor = float3 ( 0.5 , 0.5 , 0.5 ) * ( length ( normalU ) / smallVec.x ) ; 
// finalColor = float3 ( marchCount ) / 255.0 ; 
}
else
 {
...
// vignette? 
finalColor *= float3 (1.0 , 1.0 , 1.0) * saturate(1.0 - length(uv / 2.5));
finalColor *= 1.3 * exposure;

// output the final color without gamma correction - will do gamma later. 
return float3 (clamp(finalColor , 0.0 , 1.0) * saturate(fade + 0.2));
}
